...
This paper provides new graphical methods for organizing and visualizing temporal data, collected at sub-daily resolutions, into a calendar layout. It is useful to provide insights into people's daily schedules relative to events such as work days, weekends, holidays, and special events, based on calendar layouts. The calendar format is created using linear algebra, to restructure data, that can then be piped into ggplot2's plotting routines. Many options and variants are supported to make it broadly applicable.
