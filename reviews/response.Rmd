...
We recommend to make as many data plots as possible, including the calendar plots, to form a comprehensive picture about the data during the EDA process. Except for the calendar plots, the paper shows traditional displays in Figure 2, 3, and 11 as possible plots. The figure captions describe the main features that we can read from the plots. However, these typically focus on one temporal aspect, like time of day or day of week, which means that the information from other time resolutions cannot be seen. The calendar display is to complement them, laying out all possible resolutions simultaneously.
