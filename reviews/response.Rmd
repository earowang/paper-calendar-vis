...
In addition, a subsection titled "2.3.5 Faceted calendar", that describes a new function `facet_calendar()`, is included. It extends **ggplot2** intervals and complements the existing function `frame_calendar()` and allows this algorithm to integrate more deeply with the **ggplot2** architecture. We have added a subsection titled "2.4 Reasons to use calendar-based graphics" to explain why to use calendar plots.
...
We have explained the purpose of showing both displays in the revised paper, by including a new subsection "2.4 Reasons to use calendar-based graphics" to explain why to use calendar plots, and what information is typically revealed by calendar plots but not by other plots. The calendar display doesn't replace traditional displays, but serves to complement them to further tease out interesting structures in the data.
...
We have changed the explanation of Fig 11 to make it clearer. In the revised paper this is now Fig 10. The V pattern is due to morning and evening commuter pattern on weekdays. We realised that the daily lag relationships would be read more easily if local scale was used, that is, each day is scaled relative to itself. This has been done, and the different autocorrelation pattern for week days and weekends is now clearer.
...
We have added a text box underneath the plots in the shiny app. To describe the app in this paper would be too distracting for the reader. Cumulating monthly precipitation provides a more interesting look at rainfall, and it approximates how the NY Times displays weather. Only one day can be selected.
...
We have explained the concept of faceting and also referred to small multiples, trellis charts, and ensemble graphics in a new paragraph in the  Introduction section.
...
Thanks. We have pointed out the layout anomalies for May and October in the Figure 4 (now 6) caption.
...
We recommend against this idea. The better approach is to re-make the plot with global scaling to focus on counts. No need to make one plot show everything possible, but better to make multiple plots. 
