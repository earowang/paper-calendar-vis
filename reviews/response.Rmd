...
TODO. In R, a vector (a column in a data frame) only allows for one time zone. Users need to adjust these time zones to a unified one so that data over different periods can be comparable.
...
The white space between months already emphasises the separations between months. The lines separating the months are also thicker than the ones separating the days in the calendar plot.
...
We gave the range of vertical scale in the caption.
...
We have changed figure 10 (now figure 8) to align by days of the week instead of by days of the month.
