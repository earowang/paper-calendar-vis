...
We add a new short section "3 A fully-fledged faceting calendar method" describing a new function `facet_calendar()` written with **ggplot2** intervals, which complements the existing function `frame_calendar()`.
...
We add a new subsetion "2.4 Reasons to use calendar-based graphics" to explain why to use calendar plots, and what information revealed by calendar plots is but not revealed by other plots. The calendar display doesn't replace traditional displays, but serves to complement to further tease out interesting structures in the data.
...
We have switched to use the local scaling for the lag scatterplot (as Figure 10 now) to accurately represent the autocorrelation. The strong autocorrelation is clearly revealed at weekends.

We have elaborated more about the V-shape on week days.

> The colours are not well chosen in Figure 15.  It is hard to see the averages for households 1 and 3.

Done.

> There are too many large plots in the paper.  Figure 6 is not needed and Figure 12 is not needed.  How do you know that the New Year's Eve outliers in Figure 12 are from the same sensor?  That is not in the plot.

Thanks for correcting it. We have pointed out the variability instead of the outliers on New Year's Eve.
...
Thanks for the suggestion. We have restricted the data to Jan-June 2018 and combined four separate plots to a single plot using faceting.
...
Thanks. We have added text descriptions in the shiny app instead.
...
We have explained the idea of faceting and also referred to small multiples, trellis charts, and ensemble graphics at the end of the second paragraph in Introduction section.
...
Done. We deprecate argument `sunday` in favor of `week_start` that appears more general. We added a new subsection on "Start of the week" to briefly explain the option. We have reorganized the order of the subsections to be consistent with the function argument's ordering.
...
We used `tsibble::holiday_aus()` for generating Australian public holidays, prior to creating calendar displays. Whether to use holidays or not is a data-specific problem, which is beyond the scope of the calendar plot.
...
We have removed that bit from the sentence.
