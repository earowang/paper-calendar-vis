...
We dropped the boxplots as in figure 13, and the corresponding paragraph.
...
The white space between months already emphasises the separations between months. The lines separating the months are also thicker than the ones separating the days in the calendar plot. These sit in the background to make the data plot more visible, which follows the Tufte's rules.
...
We recommend to make as many data plots as possible, including the calendar plots, to form a comprehensive picture about the data during the EDA process. Except for the calendar plots, the paper shows traditional displays in Figure 2, 3, and 11 as possible plots. The figure captions describe the main features that we can read from the plots. However, these typically focus on one temporal aspect, like time of day or day of week, which means that the information from other time resolutions cannot be seen. The calendar display is not to replace but to complement them, laying out all possible resolutions simultaneously.
...
We dropped the boxplot as in figure 13, and the corresponding paragraph including this sentence.
