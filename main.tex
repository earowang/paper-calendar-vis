...
pedestrians every hour across the downtown area until recently. The
dataset can shed lights into understanding people's daily schedules, or
assisting administration and business planning. Figure
\ref{fig:facet-time} lends itself to a number of issues that make
exploratory data visualisation challenging in many temporal-context
...
\caption[Time series plot about the number of pedestrians over the year of 2016 at two train stations in the city of Melbourne]{Time series plot about the number of pedestrians over the year of 2016 at two train stations in the city of Melbourne. Small multiples of line graphs demonstrate some common periodic behaviours, for example weekly patterns, possessed by both sensors.}\label{fig:time-series-plot}
\end{figure}
\end{CodeChunk}
...
\caption[Hourly pedestrian counts facetted by sensors and days of the week using lines]{Hourly pedestrian counts facetted by sensors and days of the week using lines. It features at least two types of seasons--time of day and day of week--at both stations primarily driven by the commuting traffic. There are much more weekenders around Flinders Street Station than Flagstaff Station.}\label{fig:facet-time}
\end{figure}
\end{CodeChunk}
...
\caption[Line glyphs on the calendar format showing hourly foot traffic at Flinders Street Station, scaled over all the days]{Line glyphs on the calendar format showing hourly foot traffic at Flinders Street Station, scaled over all the days. The individual shape becomes more distinctive at the expense of the magnitude comparison loss.}\label{fig:flinders-free}
\end{figure}
\end{CodeChunk}
