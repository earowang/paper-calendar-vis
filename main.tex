...
\Abstract{
We develop the \code{frame\_calendar} function that provides a method
for arranging the data, which are collected at daily frequency or a time
scale more frequent than daily, in a calendar-based format and
accomplishes visualisation with the grammar of graphics. The
calendar-based layout serves as a familiar format for the Western world
and it is particularly useful to visualise people's daily schedules. Our
motivating application is Melbourne pedestrian counts which are captured
at hourly interval by a number of sensors. Owing to human activities,
the data feature multiple temporal scales such as time of day, day of
week, day of year. We describe its construction along with a few
options. We provide some variations to demonstrate its usage by applying
to the pedetrian data over one-year period of 2016.
}
