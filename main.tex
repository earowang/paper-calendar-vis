...
We propose a method for organising and visualising temporal data,
...
shed light into understanding people's daily schedules, or assisting
...
provides more details of the temporal patterns by faceting on day of the
week. The sensor data, like many temporal datasets on human behaviors,
lends itself to a number of exploratory data visualization challenges:
...
in 2016. The disparities between weekday and weekend along with public
...
one year, with \(M = 3\) and \(N = 4\).]{Illustration of the indexing layout for months of
one year, with \(M = 3\) and \(N = 4\).}\label{fig:year-diagram}
\end{figure}
\end{CodeChunk}
...
labels indicating weekday and month are also computed before plotting
construction.
...
\citet{Wickham2012glyph}. Figure \ref{fig:flinders-polar} is equivalent
to Figure \ref{fig:flinders-2016} placed in polar coordinates.
...
layout, which is referred to as star plots. The periodic behaviors on
workdays are clearly visible.]{Figure \ref{fig:flinders-2016} in circular
layout, which is referred to as star plots. The periodic behaviors on
workdays are clearly visible.}\label{fig:flinders-polar}
\end{figure}
\end{CodeChunk}
...
free scaling for any day within a given month. The scaling option
...
used in most countries around the world. We offer languages other than
English for text labelling. Figure \ref{fig:chn} shows the same plot as
Figure \ref{fig:boxplot} labelled using simplified Chinese characters.
...
but with the month and weekday labels in Chinese. It demonstrates the
