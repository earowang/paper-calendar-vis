...
When `polar = TRUE`, a polar transformation is carried out on the data. The computation is similar to the one described in @Wickham2012glyph. The resulting plot is star glyphs---time series lines transformed in polar coordinates---embedded in the monthly calendar layout.
...
```{r calendar-elec, fig.height = 10.5, fig.cap = "Calendar displays for each household using common scales. We can precisely tell each indidual's vacation days taking off and the overnight use for household 2 in mid January."}
```
