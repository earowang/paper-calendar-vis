...
When \code{polar = TRUE}, the polar transformation is carried out for the data.
...
The \code{frame_calendar} function does not constrain itself to mapping only temporal variable to the \code{x}. Figure \ref{fig:scatterplot} shows the lag scatterplot where hourly counts are assigned to the \code{x} and lagged themselves to the \code{y}. It provides a visual tool for identifying repeating patterns.
