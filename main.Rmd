...
The nighttime events, such as White Night and New Year Evening Firework, has barely affected the operation of Flagstaff Station but heavily affected the incoming and outgoing traffic to Flinders Street Station and State Library on these days.

## Different types of plots

The \code{frame_calendar} does not constrain itself to mapping only temporal variable to the \code{x}. Figure \ref{fig:scatterplot} shows the lag scatterplot where hourly counts are assigned to the \code{x} and lagged themselves to the \code{y}. It provides a visual tool for identifying repeating patterns.

```{r scatterplot, fig.height = 5, fig.cap = "Lag scatterplot in the daily calendar. Lagged hourly counts are plotted against current ones to demonstrate the autocorrelation at lag 1. The correlation between them is more consitent on non-work days than work days."}
```
