...
```{r time-series-plot, fig.height = 6, fig.cap = "Time series plot about the number of pedestrians in 2016 measured at 3 different sensors in the city of Melbourne. Coloured by the sensors, small multiples of lines show that the foot traffic varies from one sensor to another in terms of both time and number. The weekly patterns look distincitve across these 3 sensors. There is an eye-catching spike which occurred at State Library, caused by the annual event--White Night--on 20th of Feburary."}
```
...
A collection of data plots, organised in a familiar format, offer a useful and intuitive way to understand complex data. 
...
There are several options provided for the \code{frame_calendar} function to initialize and adjust the display of a calander plot:
...
```{r flinders-free, fig.height = 6.5, fig.cap = "Line glyphs on the calendar format showing hourly foot traffic at Flinders Street Station, scaled over all the days. The individual shape on a single day becomes more distinctive, however it is more difficult to compare the size of peaks between days."}
```
...
To avoid the overlapping problem, the calendar layout can be embedded into a series of subplots for the different sensors. Figure \ref{fig:facet} presents the idea of facetting calendar plots. This allows comparing the overall structure between sensors, while emphasising indivdual sensor variation. 
