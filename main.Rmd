...
```{r time-series-plot, fig.height = 10.5, fig.cap = "Time series plot about the number of pedestrians over the year of 2016 at a selection of 7 sensors in the city of Melbourne. Coloured by the sensors, small multiples of lines show the foot traffic varies from one sensor to another in terms of both time and number. The traffic, specifically at two train stations (Flagstaff Station and Flinders Street Station), is primarily driven by the commuters, as depicted in the graph."}
```
...
We only illustrate that grids are laid out over the horizontal direction in the paper. The vertical direction can be enabled by swapping $i$ and $j$ in the algorithm stated above. It is particularly useful for those users who get accustomed to calendars of vertical organisation in some countries.

## Scales

Section \ref{sec:grid} discusses the implementation applied to the fixed scale which is using the whole range of all the data values. In addition to the fixed scale (\code{fixed}), the remaining options include free scale on all the days (\code{free}), day of the week only (\code{free_wday}), and day of the month only (\code{free_mday}) for different comparisons.
...
```{r flinders-free, fig.height = 6.5, fig.cap = "Line glyphs on the calendar format showing hourly foot traffic at Flinders Street Station, scaled over all the days. The individual shape becomes more distinctive at the expense of the magnitude comparison loss."}
```
...
```{r scatterplot, fig.height = 5, fig.cap = "Lag scatterplot on the `daily' calendar. Lagged hourly counts are plotted against current ones to demonstrate the autocorrelation at the order of 1. The stronger association between the one-hour-ahead count and the present takes place on non-working days."}
```
