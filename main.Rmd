...
```{r flinders-polar, fig.height = 6.5, fig.cap = "Figure \\ref{fig:flinders-2016} in circular layout, which is referred to as star plots. The daily periodicity on work days are clearly visible."}
```
...
```{r flinders-free, fig.height = 6.5, fig.cap = "Line glyphs on the calendar format showing hourly foot traffic at Flinders Street Station, scaled over all the days. The individual shape on a single day becomes more distinctive, however it is impossible to compare the size of peaks between days."}
```
...
```{r dow, fig.height = 7, fig.cap = "Letter value plot of daily energy usage against day of week by households. The enery consumed by household 2 exhibits much larger variability, cotrast to the rest households. Households 1, 2, and 4 use mose energy over the weekends."}
```

```{r hod, fig.height = 6, fig.cap = ""}
```

```{r household23, fig.height = 11, fig.cap = ""}
```
