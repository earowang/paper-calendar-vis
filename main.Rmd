...
This work was originally motivated by studying foot traffic in the city of Melbourne [@ped]. There have been 43 installed sensors counting pedestrians every hour across the downtown area until recently. The dataset can shed lights into understanding people's daily schedules, or assisting administration and business planning. Figure \ref{fig:facet-time} lends itself to a number of issues that make exploratory data visualisation challenging in many temporal-context applications involving human behaviours: 
...
```{r time-series-plot, fig.height = 4.5, fig.cap = "Time series plot about the number of pedestrians over the year of 2016 at two train stations in the city of Melbourne. Small multiples of line graphs demonstrate some common periodic behaviours, for example weekly patterns, possessed by both sensors."}
```

```{r facet-time, fig.cap = "Hourly pedestrian counts facetted by sensors and days of the week using lines. It features at least two types of seasons--time of day and day of week--at both stations primarily driven by the commuting traffic. There are much more weekenders around Flinders Street Station than Flagstaff Station."}
```
