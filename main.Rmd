...
The algorithm has several optional parameters that modify the layout, direction of display, scales, white space and switching to polar coordinates. These are accessible to the user by the inputs to the function  \code{frame_calendar}:
...
By default, grids are laid out horizontally. This can be transposed by setting the \code{dir} parameter to \code{"v"}, in which case $i$ and $j$ are swapped in the equation \eqref{eq:grid}. This can be useful for creating  calendar layouts for countries where vertical layout is the convention.
